...
void CSimpleTileMap::Render() const
{
    ...
    for (int y = 0; y < m_mapSize; y++)
    {
        for (int x = 0; x < m_mapSize; x++)
        {
            ...
            App::DrawQuad(Vector2(xPos, yPos), Vector2(xPos + w, yPos + h), Vector3(g_tileMapDef[index][0], g_tileMapDef[index][1], g_tileMapDef[index][2]));
        }
    }
}

int CSimpleTileMap::GetTileIndexForTilePos(Vector2Int tilePos)
{
    int index = tilePos.y * m_mapSize + tilePos.x;

    return index;
}

int CSimpleTileMap::GetTileIndexForWorldPos(Vector2 worldPos)
{
    Vector2Int tilePos = GetTilePosForWorldPos(worldPos);
    int tileIndex = GetTileIndexForTilePos(tilePos);

    return tileIndex;
}

Vector2Int CSimpleTileMap::GetTilePosForTileIndex(int tileIndex)
{
    Vector2Int tilePos = Vector2Int(tileIndex % m_mapSize, tileIndex / m_mapSize);

    return tilePos;
}

Vector2Int CSimpleTileMap::GetTilePosForWorldPos(Vector2 worldPos)
{
    worldPos.x += (m_tileWidth * 0.5f);

    int tilex = (int)floor(worldPos.x / m_tileWidth);
    int tiley = (int)floor(worldPos.y / m_tileHeight);

    return Vector2Int(tilex, tiley);
}

Vector2 CSimpleTileMap::GetWorldPosForTileIndex(int tileIndex)
{
    Vector2Int tilepos = GetTilePosForTileIndex(tileIndex);

    Vector2 WorldPos = Vector2(tilepos.x * m_tileWidth, tilepos.y * m_tileHeight);

    return WorldPos;
}

Vector2 CSimpleTileMap::GetWorldPosForTilePos(Vector2Int tilePos)
{
    Vector2 worldPos = Vector2(tilePos.x * m_tileWidth, tilePos.y * m_tileHeight);
...
bool CSimpleTileMap::IsTileWalkableForWorldPos(Vector2 worldPos)
{
    Vector2Int tilePos = GetTilePosForWorldPos(worldPos);
...
bool CSimpleTileMap::IsTileWalkableForTilePos(Vector2Int tilePos)
{
    if (tilePos.x < 0 || tilePos.x >= m_mapSize
        || tilePos.y < 0 || tilePos.y > m_mapSize)
        return false;

    if (GetTileMapValue(tilePos.x, tilePos.y) != EMapValue::FLOOR)
    {
        return false;
    }
